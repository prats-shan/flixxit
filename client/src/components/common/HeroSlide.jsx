...
const HeroSlide = ({ mediaType, mediaCategory }) => {
  ...
  useEffect(() => {
    const getMedias = async () => {
      const { response, err } = await mediaApi.receiveList ({
        mediaType,
        mediaCategory,
        page: 1
      });
    ...
    const getAllGenres = async () => {
      ...
      const { response, err } = await genreApi.receiveList ({ mediaType });
        ...
        {movies.map((movie, index) => (
          <SwiperSlide key={index}>
            <Box sx={{
              ...
              backgroundImage: `url(${tmdbConfigs.backdropDirection(movie.backdrop_path || movie.poster_path)})`
            }} />
            <Box sx={{
              width: "100%",
              height: "100%",
