...
import { clearFromFavorite } from "../redux/features/userSlice";
...
const FavoriteItem = ({ media, onRemoved }) => {
  ...
  const onRemove = async () => {
    if (onRequest) return;
    setOnRequest(true);
    const { response, err } = await favoriteApi.clear({ favoriteId: media.id });
    setOnRequest(false);

    if (err) toast.error(err.message);
    if (response) {
      toast.success("Remove favorite success");
      dispatch(clearFromFavorite({ mediaId: media.mediaId }));
      onRemoved(media.id);
    }
  };
...
const FavoriteList = () => {
  ...
  useEffect(() => {
    const getFavorites = async () => {
      ...
      const { response, err } = await favoriteApi.receiveList ();
