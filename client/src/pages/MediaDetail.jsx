...
import { addFavorite, clearFromFavorite, removeFromFavorite } from "../redux/features/userSlice";
...
const MediaDetail = () => {
  ...
  useEffect(() => {
    ...
    const getMedia = async () => {
      ...
      const { response, err } = await mediaApi.receiveDetail({ mediaType, mediaId });
  ...
  const onRemoveFromFavorite = async () => {
    ...
    if (err) toast.error(err.message);
    if (response) {
      dispatch(clearFromFavorite(favorite));
      setIsFavorite(false);
  ...
  return (
    media ? (
      <>
        <ImageHeader imgPath={tmdbConfigs.backdropDirection(media.backdrop_path || media.poster_path)} />
        <Box sx={{
